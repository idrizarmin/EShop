...
class ProductByCategoryProvider extends ChangeNotifier {
  ...
  filterInitialProductAndSubCategory(Category selectedCategory){
    ...
    filteredProduct = _dataProvider.products.where((product)=> product.proCategoryId?.name == selectedCategory.name).toList();
 ...
 filterProductBySubCategory(SubCategory subCategory) {
    ...
    if(subCategory.name?.toLowerCase()== 'all'){
      filteredProduct = _dataProvider.products.where((product)=> product.proCategoryId?.name == mySelectedCategory?.name).toList();
      brands= [];
    }else{
      filteredProduct = _dataProvider.products.where((product)=> product.proSubCategoryId?.name == subCategory.name).toList();
      brands = _dataProvider.brands.where((brand)=> brand.subcategoryId?.sId== subCategory.sId).toList();
    }
    notifyListeners();
 }
  ...
  filterProductByBrand(){
    if(selectedBrands.isEmpty){
      ...
      filteredProduct = _dataProvider.products.where((product)=> product.proSubCategoryId?.name == mySelectedSubCategory?.name && 
