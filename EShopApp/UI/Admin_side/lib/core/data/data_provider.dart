...
class DataProvider extends ChangeNotifier {
  ...
  Future<List<Variant>> getAllVriants({bool showSnack = false}) async {
    try {
  ...
  void filterProducts(String keyword) {
   ...
   Future<List<Coupon>> getAllCupons({bool showSnack = false}) async {
    try {
  ...
  void filterCupons(String keyword) {
    if (keyword.isEmpty) {
      _filteredCoupons = List.from(_allCoupons);
    } else {
      final lowerKeyword = keyword.toLowerCase();
      _filteredCoupons = _allCoupons.where((cupon) {
        return (cupon.couponCode?? '').toLowerCase().contains(lowerKeyword);
      }).toList();
    }
    notifyListeners();
  }
...
Future<List<Order>> getAllOrders({bool showSnack = false}) async {
  ...
  void filterOrders(String keyword) {
    if (keyword.isEmpty) {
      ...
      _filteredOrders = _allOrders. where((order) {
        bool nameMatches = (order.userID?.name ?? '').toLowerCase().contains(keyword);
 ...
 int calculateOrdersWithStatus({String? status}) {
  ...
  void filterProductsByQuantity(String productQutType) {
    if (productQutType == 'All Product') {
      _filteredProducts = List.from(_allProducts);
    } else if (productQutType == 'Out of Stock') {
      _filteredProducts = _allProducts.where((product) {
        return product.quantity != null && product.quantity == 0;
      }).toList();
    } else if (productQutType == 'Limited Stock') {
      _filteredProducts = _allProducts.where((product) {
        return product.quantity != null && product.quantity == 1;
      }).toList();
    } else if (productQutType == 'Other Stock') {
      _filteredProducts = _allProducts.where((product) {
